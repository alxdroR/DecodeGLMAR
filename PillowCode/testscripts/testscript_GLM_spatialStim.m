...
ktmu = normalizecols([mean(ggsim.k,2),mean(gg1.k,2)]);
kxmu = normalizecols([mean(ggsim.k)',mean(gg1.k)']);
Errs_T = [subspace(ktmu(:,1),ktmu(:,2))]
Errs_X = [subspace(kxmu(:,1),kxmu(:,2))]

errfun = @(x,y)(sum((x(:)-y(:)).^2));
Errs_Total = [errfun(ggsim.k,gg1.k), ]
